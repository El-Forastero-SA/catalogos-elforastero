#!/usr/bin/env python3
"""
generar_catalogo.py — Genera catalogo HTML de productos desde Magento 2 API.

Conecta al API REST de Magento 2, obtiene productos de una categoria,
y genera un archivo HTML listo para conversion a PDF (1080x1920px).

Uso:
    python3 generar_catalogo.py --category-id 111 --category-name "Cafe" --category-color "#5C3D2E"
...
def magento_get(base_url, token, endpoint):
    """Hace un GET al API REST de Magento 2. Retorna dict parseado."""
    url = f"{base_url.rstrip('/')}/rest/V1/{endpoint.lstrip('/')}"
    ...
    try:
        ...
        try:
            ...
            pass
        print(f"Error API Magento (HTTP {e.code}): {e.reason}", file=sys.stderr)
        if body:
            print(f"  Detalle: {body[:500]}", file=sys.stderr)
        sys.exit(1)
    except urllib.error.URLError as e:
        print(f"Error de conexion a Magento: {e.reason}", file=sys.stderr)
        sys.exit(1)


def fetch_category_name(base_url, token, category_id):
    """Obtiene el nombre de una categoria por ID."""
    data = magento_get(base_url, token, f"categories/{category_id}")
    return data.get("name", f"Categoria {category_id}")


def fetch_products(base_url, token, category_id):
    """Obtiene productos habilitados de una categoria."""
    # Construir query con brackets URL-encoded
    ...
    endpoint = f"products?{params}"
    data = magento_get(base_url, token, endpoint)

    items = data.get("items", [])

    # Filtrar solo productos habilitados (status=1)
    products = []
    for item in items:
        ...
        # Extraer imagen de custom_attributes (fallback: small_image, thumbnail)
        image_attrs = {}
        ...
        for attr_name in ("image", "small_image", "thumbnail"):
            path = image_attrs.get(attr_name)
            if path and path != "no_selection":
                image_url = f"{base_url.rstrip('/')}/pub/media/catalog/product{path}"
                break
    ...
    excluded = total - len(products)
    if excluded:
        print(f"  Productos sin imagen excluidos: {excluded}")
...
def split_product_name(name):
    ...
    """
    # Intentar separar por " - " (ultimo)
    idx_dash = name.rfind(" - ")
    # Intentar separar por " x " (ultimo, case insensitive para " X ")
    idx_x = name.lower().rfind(" x ")

    # Usar el separador que este mas hacia el final (mayor indice)
    split_idx = max(idx_dash, idx_x)

    if split_idx > 0 and split_idx < len(name) - 3:
        sep_len = 3  # " - " o " x " tienen longitud 3
        line1 = name[:split_idx].strip()
        line2 = name[split_idx + sep_len:].strip()
        return line1, line2

    # Si no se encontro separador, retornar nombre completo en linea 1
    return name, ""
...
# SVG background pattern for content areas (from template)
SVG_BG_PATTERN = (
...
def main():
    parser = argparse.ArgumentParser(
        description="Genera catalogo HTML de productos desde Magento 2 API."
    )
    parser.add_argument(
        "--category-id",
        required=True,
        type=int,
        help="ID de la categoria en Magento",
    )
    parser.add_argument(
        "--category-name",
        default=None,
        help="Nombre para mostrar en el catalogo (default: nombre de la API)",
    )
    parser.add_argument(
        "--category-color",
        default="#BC1221",
        help="Color hex para la categoria (default: #BC1221)",
    )
    parser.add_argument(
        "--output",
        default=None,
    ...
    parser.add_argument(
        "--pdf",
        action="store_true",
        help="Generar tambien PDF (requiere weasyprint instalado)",
    )
    ...
    if not category_name:
        print(f"Obteniendo nombre de categoria {args.category_id}...")
        category_name = fetch_category_name(base_url, token, args.category_id)

    print(f"Categoria: {category_name}")

    # --- Obtener productos ---
    print(f"Obteniendo productos de categoria {args.category_id}...")
    products = fetch_products(base_url, token, args.category_id)
    print(f"Productos encontrados (habilitados): {len(products)}")

    if not products:
        print("No se encontraron productos habilitados. Abortando.", file=sys.stderr)
        sys.exit(1)
    ...
    # Si es ruta relativa, guardar en el directorio del script
    ...
    if args.pdf:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                page.goto(f"file://{output_path}", wait_until="networkidle", timeout=120000)
                # Esperar a que carguen las imagenes
                page.wait_for_timeout(3000)
                # Contar paginas del catalogo
            ...
            print("  Instalar con: pip3 install playwright && python3 -m playwright install chromium", file=sys.stderr)
            pdf_path = None
